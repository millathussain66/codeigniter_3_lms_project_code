...
class PHPExcel_Shared_String
{
    ...
    {
        if (self::getIsMbstringEnabled()) {
            return mb_strlen(strval($value), $enc);
        }

        if (self::getIsIconvEnabled()) {
            return iconv_strlen(strval($value), $enc);
        }
    ...
    {
        if (self::getIsMbstringEnabled()) {
            return mb_substr(strval($pValue), $pStart, $pLength, 'UTF-8');
        }

        if (self::getIsIconvEnabled()) {
            return iconv_substr(strval($pValue), $pStart, $pLength, 'UTF-8');
        }

        // else substr
        return substr(strval($pValue), $pStart, $pLength);
    }
    ...
    public static function StrToUpper($pValue = '')
    {
        if (function_exists('mb_convert_case')) {
            return mb_convert_case(strval($pValue), MB_CASE_UPPER, "UTF-8");
        }
    ...
    {
        if (function_exists('mb_convert_case')) {
            return mb_convert_case(strval($pValue), MB_CASE_LOWER, "UTF-8");
        }
        return strtolower(strval($pValue));
    }
    ...
    {
        if (function_exists('mb_convert_case')) {
            return mb_convert_case(strval($pValue), MB_CASE_TITLE, "UTF-8");
        }
        return ucwords(strval($pValue));
    }

    public static function mb_is_upper($char)
    {
        return mb_strtolower(strval($char), "UTF-8") != $char;
    }
