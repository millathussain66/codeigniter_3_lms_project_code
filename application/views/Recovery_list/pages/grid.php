	...
	<div id="body">
		...
		<script type="text/javascript">
			...
			jQuery(document).ready(function($) {
				...
				var source = {
					...
					url: '<?= base_url() ?>index.php/Recovery_list/grid',
					cache: false,
					filter: function() {
					...
					columns: [{
						...
						<? if (SENDTOCHECKER == 1) { ?> {
								...
								cellsrenderer: function(row) {
									...
									return '<div style="text-align:center;margin-top: 5px;  cursor:pointer" onclick="delete_record_message(' + dataRecord.id + ',\'sendtochecker\',' + editrow + ',' + dataRecord.sts + ');" ><img align="center" src="<?= base_url() ?>images/forward.png"></div>';
			...
			function editt2(val, indx) {
				...
				EOL.messageBoard.open('<?= base_url() ?>Recovery_list/from2/edit/' + val + '/' + indx, (jQuery(window).width() - 80), jQuery(window).height(), 'yes');
				return false;
			}
			...
			function delete_records() {
				...
				jQuery.ajax({
					type: "POST",
					cache: false,
					url: '<?= base_url() ?>index.php/Recovery_list/delete_action/',
					data: postData,
					datatype: "json",
		...
		<div style="padding: 0.5%;width:98%;height:30px; border:1px solid #c0c0c0;font-family: Calibri;font-size: 14px">
			<table id="deal_body" style="display:block;width:100%">
				<tr>
					...
					<td style="text-align:right;width:10%"><strong>CID Number &nbsp;&nbsp;</strong> </td>
					<td style="width:15%"><input type="text" name="case_number" id="case_number" placeholder="CIF Number"></td>
		...
		<div style="float:left;padding-top: 5px;padding-left:5px;">
			<div style="font-family: Calibri; margin: 0 0 -10px 0;font-size:14px;color:#0000cc">
				<? if (ADD == 1) { ?>
					<a style="text-decoration:none" onclick="javascript:EOL.messageBoard.open(this.href, (jQuery(window).width()-70), jQuery(window).height(), 'yes'); return false;" href="<?= base_url() ?>index.php/Recovery_list/from/add" title=""><input type="button" class="buttonStyle" style="background-color:#c2c2bc;font-size:13px;border-radius: 3px !important;height:40px;width:120px;" value="Upload" id="sendButton" /></a>
