...
<script type="text/javascript">
	...
	var checker_info = [<? $i = 1;
						foreach ($checker_info as $row) {
							if ($i != 1) {
								echo ',';
							}
	...
	jQuery(document).ready(function() {
		...
		rules2 = [{
			...
			rule: function(input) {
				if (input.val() != '') {
					...
					if (item != null) {
						jQuery("input[name='checker']").val(item.value);
					}
					return true;
				} else {
		...
		jQuery("#checker").jqxComboBox({
			theme: theme,
			autoOpen: false,
			autoDropDownHeight: false,
			promptText: "Select Legal Checker",
			source: checker_info,
			width: 250,
			height: 25
		});
		...
		<? } ?>
		jQuery('#checker,#legal_zone').focusout(function() {
	...
	<div style="width:96%;margin: auto;paddding: 10px;">
		...
		<form class="form" name="sendtolegalform" id="sendtolegalform" method="post" action="#">
			...
			<table class="custtable" border="0" style="table-layout: fixed;margin-left:10px;margin-top:10px;width:99%; font-size:15px;border-collapse:collapse">
									...
									<?php if (!empty($facility_info)) : ?>
										...
										foreach ($facility_info as $key) : ?>
											<tr>
												...
												<td>
													<input type="text" tabindex="3" style="width:100px" name="belance_dt_<?= $cot; ?>" placeholder="dd/mm/yyyy" id="belance_dt_<?= $cot; ?>" value="">
													<script type="text/javascript" charset="utf-8">
														datePicker("belance_dt_<?= $cot; ?>");
													</script>
												</td>
												<td>
				...
				<tr>
					<td style="width:12%"><strong>Select Zone</strong></td>
					<td style="width:30%">
						<div id="legal_zone" name="legal_zone" onchange="get_user_data_region_wise()" style="padding-left: 3px;margin: 0 auto;" tabindex="1"></div>
					</td>
