<div id="container">
	...
	<script type="text/javascript">
         jQuery(document).ready(function () {
           ...
           <? if(ADD==1){?>
            ...
            var source =
            {
			    ...
			    url: '<?=base_url()?>index.php/Menu_group/grid',
				cache: false,
				filter: function()
				{
            ...
            jQuery("#jqxgrid").jqxGrid(
            {
			    ...
			    columns: [
					  ...
					  { text:'Sort Name', datafield: 'sort_name',  editable: false,  width: '15%' },
					  { text:'Menu Group Name', datafield: 'menu_name',  editable: false,  width: '45%' },
		...
		function editt(val,indx)
		{
			...
			EOL.messageBoard.open('<?=base_url()?>index.php/Menu_group/from/edit/'+val+'/'+indx, (jQuery(window).width()-70), jQuery(window).height(), 'yes');
			return false;
		}
		...
		function initDeleteMessageDialog(val,row) {
		  ...
		  var handleSubmit = function() {
			...
			var request =  new Request({	url: '<?=base_url()?>index.php/Menu_group/delete_action',
											method: 'post',
    ...
    href="<?=base_url()?>index.php/Menu_group/from/add" title=""><input type="button" class="buttonStyle"  value="Add Menu Group" id="sendButton" /></a>
    <? }?>
	</div>
