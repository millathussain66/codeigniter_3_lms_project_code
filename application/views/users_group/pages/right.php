	...
	jQuery(document).ready(function() {
		...
		jQuery("#btnSavePurchaseEntry").click(function() {
				...
				jQuery.ajax({
						type: "POST",
						cache: false,
						url: "<?=base_url()?>users_group/set_right_update/<?=$wg_Id?>",
						data : postdata,
						datatype: "json",
...
<form method="post" action="#" style="margin: 30px;" id="form">
	...
	<tr>
		<td style="text-align: left;" width="49%"><input type="checkbox" id="check-all" ><strong>Select All </strong></td>
		<td style="text-align: right;" width="49%"><strong><?=$name?></strong></td>
	</tr>
	</table><br />
					<?php
						$GROUP_ID = 0;
						...
						foreach($result as $row)
						{
							...
							<td width="25%">
								<input type="checkbox" name="group<?=$i?>categ<?=$j?>input<?=$k?>" id="group<?=$i?>categ<?=$j?>input<?=$k?>" class="group<?=$i?>categ<?=$j?>input"  /><?=$row['right_name']?>
