...
class Case_against_bank extends CI_Controller {
	...
	function view ($menu_group,$menu_cat,$menu_link=null,$submenu=null)
	{
		...
		$data = array(
			...
			'req_type'=>$this->User_model->get_parameter_data('ref_case_type','name','data_status=1'),
			...
			'plaintiff' =>$this->User_model->get_parameter_data('users_info','name','data_status = 1  AND admin_status<>2 AND user_group_id IN(4,6,7) AND block_status = 0'),
	...
	function make_xl()
	{
        ...
        DATE_FORMAT(s.filling_date,'%d-%b-%y') AS filling_date,bs.name as branch_name,
        ...
        s.remarks,co.total_cost as total_legal_cost
	        ...
	        $headings4 = array('Sl No','A/C Number','A/C Name',
	    		'Branch Name',
	        	'Type Of Case','Filling Date','Case Number','Previous Date','Case Status On The Previous','District','Final Remarks','Legal Cost');//,strtotime($dealdate)));
