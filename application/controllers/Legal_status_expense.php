...
class Legal_status_expense extends CI_Controller
{
	...
	function search_suit()
	{
		...
		$suit_row = $this->db->query("SELECT r.name as req_type,c.case_number,c.id,c.loan_ac,c.ac_name
			...
			LEFT OUTER JOIN ref_branch_sol bs on(bs.code=c.branch_sol)
	...
	function get_add_info()
	{
		...
		if ($req_type == 2 && $row_info->case_name == 4) {
			...
			$expense_activities = $this->User_model->get_parameter_data('ref_schedule_charges_ara', 'name', 'data_status = 1 AND (aurtho_jari_sts=0 OR aurtho_jari_sts IS NULL)');
		} else {
	...
	function details()
	{
		...
		if (!empty($details)) {
			...
			$solanama_document_upload = '<img id="file_preview" onclick="popup(\'' . base_url() . 'cma_file/solanama_document_upload/' . $details->solanama_document_upload . '\')" style=" cursor:pointer;text-align:center" height="18" src="' . base_url() . 'old_assets/images/print-preview.png">';
			...
			$str .= '<tr>
				...
				<tr>
					<td width="50%" align="left"><strong>Solenama Date :</strong>' . $details->solenama_date . '</td>
					<td width="50%" align="left"><strong>Probation Date:</strong>' . $details->probation_date . '</td>
				</tr>

			<tr>
				<td width="50%" align="left"><strong>Solanama Document:</strong>' . $solanama_document_upload . '</td>
				<td width="50%" align="left"><strong>Court Fee Amount :</strong>' . $details->court_fee_amount . '</td>
			</tr>
	...
	function delete_action()
	{
		...
		if (count($text) <= 0) {
			...
			if ($id == 'taken') {
				...
				$row = $this->Legal_status_expense_model->get_add_action_data($id);
			}
		} else {
			for ($i = 0; $i < count($text); $i++) {
				if ($i > 0) {
