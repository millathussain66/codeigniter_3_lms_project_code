<div id="container">
	...
	<script type="text/javascript">
         jQuery(document).ready(function () {
           ...
           <? if(ADD==1){?>
            ...
            var source =
            {
			    ...
			    url: '<?=base_url()?>index.php/Menu_category/grid',
				cache: false,
				filter: function()
				{
            ...
            jQuery("#jqxgrid").jqxGrid(
            {
			    ...
			    columns: [
					  	...
					  	{ text: 'D', menu: false,columntype: 'number',sortable: false, width: 30, align:'center', cellsalign:'center',
						  	cellsrenderer: function (row) {
								...
								var dataRecord = jQuery("#jqxgrid").jqxGrid('getrowdata', editrow);
							 		...
							 		return '<div style="text-align:center;  cursor:pointer" onclick="delete_action('+editrow+','+dataRecord.menu_group_id+','+dataRecord.id+')" ><img align="center" src="<?=base_url()?>images/delete.png"></div>';
					  ...
					  { text: 'E', menu: false, datafield: 'Edit', columntype: 'number',  sortable: false, width: 30, align:'center', cellsalign:'center',
					   ...
					   <? }?>					  
                      { text:'Menu Group Name', datafield: 'menu_name',  editable: false,  width: '30%' },
                      { text:'Menu Category name', datafield: 'menu_cate_name',  editable: false,  width: '30%' },
					  { text:'URL Prefix', datafield: 'url_prefix',  editable: false,  width: '20%' },
                      { text:'Has Child', datafield: 'has_child', editable: false, width: '8%'},
		...
		function delete_action(row,val,valId){

			jQuery("#val").val(val);
			jQuery("#val1").val(valId);
			jQuery("#row").val(row);
		...
		function editt(val,indx)
		{
			...
			EOL.messageBoard.open('<?=base_url()?>index.php/Menu_category/from/edit/'+val+'/'+indx, (jQuery(window).width()-70), jQuery(window).height(), 'yes');
			return false;
		}
		...
		function initDeleteMessageDialog(val,row) {
		  ...
		  var handleSubmit = function() {
			...
			var val1=jQuery("#val1").val();  
			var request =  new Request({	url: '<?=base_url()?>index.php/Menu_category/delete_action',
											method: 'post',
											data: {'<?php echo $this->security->get_csrf_token_name(); ?>':csrf_token,grp_id:val,id:val1},
    ...
    href="<?=base_url()?>index.php/Menu_category/from/add" title=""><input type="button" class="buttonStyle"  value="Add Menu Category" id="sendButton" /></a>
    <? }?>
	</div>
