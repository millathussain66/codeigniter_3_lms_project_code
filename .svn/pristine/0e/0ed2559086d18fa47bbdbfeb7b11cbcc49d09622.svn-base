...
class Legal_status_expense_model extends CI_Model
{
    ...
    function get_grid_data($filterscount, $sortdatafield, $sortorder, $limit, $offset)
    {
        ...
        $branch_user_array = array("6","4","9","7","8");
        $head_office_array = array("2","3");
        if (in_array($this->session->userdata['ast_user']['user_work_group_id'], $branch_user_array)) {
        ...
        $this->db
            ...
            j1.name as status,CONCAT(bs.name," (",bs.code,")") as branch_name,
            z.name as zone_name,
            bs.name as branch_name,
            rq.name as case_type,
            CONCAT(j2.name," (",j2.user_id,")")as e_by,
            CONCAT(j3.name," (",j3.user_id,")")as u_by,
            DATE_FORMAT(j0.e_dt,"%d-%b-%y %h:%i %p") AS e_dt,
            IF('.$this->session->userdata['ast_user']['user_work_group_id'].'<>4 AND '.$this->session->userdata['ast_user']['user_work_group_id'].'<>6 AND '.$this->session->userdata['ast_user']['user_work_group_id'].'<>9 AND j0.sts=37,1,j1.s_order) as priority_order,
            DATE_FORMAT(j0.u_dt,"%d-%b-%y %h:%i %p") AS u_dt', FALSE)
            ->from("change_request j0")
            ->join('suit_filling_info as c', 'c.id=j0.suit_file_id', 'left')
            ->join('ref_branch_sol as bs', 'bs.code=c.branch_sol', 'left')
            ->join('ref_zone as z', 'z.id=c.zone', 'left')
            ->join('ref_req_type as rq', 'rq.id=c.req_type', 'left')
