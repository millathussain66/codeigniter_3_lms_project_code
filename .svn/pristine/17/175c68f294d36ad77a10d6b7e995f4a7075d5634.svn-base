...
<script type="text/javascript">
	...
	function search_data()
	{
		...
		jQuery.ajax({
				type: "POST",
				cache: false,
				url: "<?=base_url()?>index.php/authorization_request/load_bulk_data/",
				data : postdata,
				datatype: "json",
	...
	function details_sfa(id,operation,type,event_id)
	{
		jQuery('#deleteEventId').val(id);
		jQuery('#type').val(type);
		jQuery('#event_id').val(event_id);
	...
	function call_ajax_submit()
	{
		...
		jQuery.ajax({
			type: "POST",
			cache: false,
			url: '<?=base_url()?>index.php/authorization_request/bulk_acktion/',
			data : postData,
			datatype: "json",
			success: function(response){
			...
			<div style="margin-left:10px;padding: 0.5%;width:98%;height:65px; border:1px solid #c0c0c0;font-family: Calibri;font-size: 14px">
					<table id="deal_body" style="display:block;width:100%">
						<tr>
							...
							<td style="width:5%"><input id="req_dt_from" name="req_dt_from" style="width:40%" /><script type="text/javascript">datePicker("req_dt_from");</script>
