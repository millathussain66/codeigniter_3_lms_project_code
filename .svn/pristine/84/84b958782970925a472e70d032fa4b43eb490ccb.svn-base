...
class Warrant_arrest_model extends CI_Model {
	...
	function get_grid_data($filterscount,$sortdatafield,$sortorder,$limit, $offset)
	{
    	...
    	$this->db->select('SQL_CALC_FOUND_ROWS s.*, s.id as suit_id,s.cma_id,s.case_name, s.case_number,s.case_claim_amount,s.proposed_type,s.loan_ac,s.ac_name,s.legal_region,
    	n.name as case_n,z.name as zone_name,bs.name as branch_name,rq.name as case_type, o.guarantor_name as owner_name,o.guarantor_type as owner_type,cs.name as case_sts,f.id as wa_id,f.v_sts as wa_v_sts,f.status as setteled_sts,cr.name as executed_criterea_name
    	', FALSE)
    ...
    function add_edit_action($add_edit=NULL,$edit_id=NULL,$editrow=NULL)
	{
		...
		if(isset($_POST['setteled'])){
			...
			$expenses['impresonment_month']=$this->security->xss_clean( $this->input->post('impresonment_month'));
			$expenses['debtor_name']=$this->security->xss_clean( $this->input->post('debtor_name'));
