...
class PHPExcel_Calculation_Financial {	
 ...
 //return strtotime($f);
	...
	public static function T_YIELD($settlement, $maturity, $rate, $price, $redemption, $frequency, $basis) {
...
//		 $e =180;// 
	...
	public static function gghYIELD($settlement, $maturity, $rate, $price, $redemption, $frequency, $basis) {
		...
		//$a = self::COUPDAYBS($settlement, $maturity, $frequency, $basis);
		//$a=round(trim($a)); // this line added by hosain 22-12-2020
		
		$coupon=$rate;
		$price=$price;
		$red=$redemption;
		$freq=$frequency;
		//return '$n:'.$n.'$dsc:'.$dsc.'$e:'.$e.'$a:'.$a;		
...
//print("$i\n");
	...
	public static function hYIELD($settlement, $maturity, $rate, $price, $redemption, $frequency, $basis) {
...
//print("$i\n");
	...
	public static function YEARFRAC_NRB($issue, $settlement,$basis) 
	{
		//return PHPExcel_Calculation_DateTime::YEARFRAC_NRB($issue, $settlement, $basis);
	...
	public function get_hp_interest($rate,$hp,$FV,$dtdiff,$formula_ebl=0)
	{
		if($formula_ebl==0)
		{
			$v1=($rate * $hp * $FV);
			...
			$hpi =round( (100 * ($rate/100) * $hp )/ ($dtdiff * 2) , $formula_ebl) * ($FV/100);
		}
		return $hpi;
	}
	
	public function get_hp_interest_prov($rate,$hp,$FV,$dtdiff)
	{
		$hpi = ($rate * $hp * $FV) / (100 * $dtdiff * 2);		
		//$hpi =round( (100 * ($rate/100) * $hp )/ ($dtdiff * 2) , 6) * ($FV/100);	
	...
	public function b2_get_tenor_from_yearfraction($yearfraction,$dealtype)
	{
		...
		{
			// Not sure .. call to muntasir and discussed regarding below codes
			...
			if($yearfraction<=7){$tnor=7;}
			else if($yearfraction>7 && $yearfraction<=14){$tnor=14;}
			else if($yearfraction>14 && $yearfraction<=30){$tnor=30;}
			else if($yearfraction>30 && $yearfraction<=91){$tnor=91;}
			else if($yearfraction>91 && $yearfraction<=182){$tnor=182;}
			else if($yearfraction>182 && $yearfraction<=364){$tnor=364;}		
			else {$tnor=364;}
