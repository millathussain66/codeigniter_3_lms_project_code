...
<script type="text/javascript">
        ...
        ?>
    var month = [<? $i=1; foreach($months as $key=> $row){ if($i!=1){echo ',';} echo '{value:"'.$key.'", label:"'.$row.'"}'; $i++;}?>];
    var year = [<? $i=1; for($j=date('Y');$j>=1990;$j--){ if($i!=1){echo ',';} echo '{value:"'.$j.'", label:"'.$j.'"}'; $i++;}?>];

    jQuery(document).ready(function () {

        jQuery("#month").jqxComboBox({theme: theme,  autoOpen: false,autoDropDownHeight: false, promptText: "Select Month", source: month, width: 250, height: 25});
       ...
       jQuery("#year").jqxComboBox({theme: theme,  autoOpen: false,autoDropDownHeight: false, promptText: "Select Year", source: year, width: 150, height: 25});
        jQuery('#month,#year').focusout(function() {
...
function search_data(){
    ...
    else
    {
        ...
        jQuery.ajax({
            ...
            cache: false,
            async:false,
            url: "<?=base_url()?>index.php/wa_rt/case_scenario_rt/",
            data : postdata,
            datatype: "html",
        ...
        <table class="">
            <tr id="widgetsNavigationTree">
                ...
                <td valign="top" id="demos" class='rc-all content'  style="width:85%;overflow: hidden;">
                    <div id="search_area">
                    <form method="POST" name="form" id="form" action="<?php echo base_url('wa_rt/case_scenario_rt_xl'); ?>"  style="margin:0px;">
                       ...
                       <div style="padding: 0px;width:100%;height:50px; border:1px solid #c0c0c0;font-family: Calibri;font-size: 14px">
                            <table id="deal_body" style="display:block;width:100%">
                                <tr>
                                    ...
                                    <td><button type='submit' formtarget="_blank" name='xl_search' id="xl_search" value='Search' style="width:58px;border: none;background: transparent;display: none;"><img src="<?=base_url()?>images/icon_xls.gif" width="20px" height="20px"></button>
                                    </td>
                                </tr>
                            </table>
                      </div>
