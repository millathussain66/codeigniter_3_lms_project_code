...
.back_image{
    ...
    <script type="text/javascript">
            ...
            var branch_sol = [<? $i=1; foreach($branch_sol as $row){ if($i!=1){echo ',';} echo '{value:"'.$row->code.'", label:"'.$row->name.'"}'; $i++;}?>];
            ...
            jQuery("#branch_sol").jqxComboBox({theme: theme,  autoOpen: false,autoDropDownHeight: false, promptText: "Select Branch", source: branch_sol, width: 215, height: 25});
           ...
           jQuery("#sendButton").jqxButton({template:"primary",width:"170"});
            ...
            jQuery('#notification_type,#zone,#lawyer,#bank,#branch').focusout(function() {
           ...
           datafields: [
            ...
            jQuery("#jqxgrid").jqxGrid(
            {
                ...
                columns: [
                        ...
                        <? if(ACKOWLEDGMENTREQUEST==1){?>
                          { text: 'ACK', datafield: 'AcknowledgementRequest', editable: false,align:'center', sortable: false, menu: false, width: 35,
                            cellsrenderer: function(row) {
                                ...
                                if((dataRecord.cma_sts == 3 || dataRecord.cma_sts==87)){
                        ...
                        <? if(REASSIGN==1){?>
                          { text: 'RF', datafield: 'reassign', editable: false,align:'center', sortable: false, menu: false, width: 35,
                            cellsrenderer: function(row) {
                                editrow = row;
                                var dataRecord = jQuery("#jqxgrid").jqxGrid('getrowdata', editrow);
                                if((<?=$this->session->userdata['ast_user']['user_id']?>==dataRecord.legal_user || <?=$this->session->userdata['ast_user']['user_system_admin_sts']?>=='2') && (dataRecord.cma_sts ==61 || dataRecord.cma_sts==62 || dataRecord.cma_sts==63 || dataRecord.cma_sts==67 || dataRecord.cma_sts==69 || dataRecord.cma_sts==64)){
                        ...
                        <? if(APPROVEREASSIGN==1){?>
                          { text: 'AR', datafield: 'approvereassign', editable: false,align:'center', sortable: false, menu: false, width: 35,
                            cellsrenderer: function(row) {
                                editrow = row;
                                var dataRecord = jQuery("#jqxgrid").jqxGrid('getrowdata', editrow);
                                if((<?=$this->session->userdata['ast_user']['user_work_group_id']?>==1 || <?=$this->session->userdata['ast_user']['user_system_admin_sts']?>=='2') && dataRecord.cma_sts == 84 ){
                        ...
                        <? if(LAWYERNOTIFICATIONSENT==1){?>
                          { text: 'SNL', datafield: 'lawyernotificationsent', editable: false,align:'center', sortable: false, menu: false, width: 35,
                            cellsrenderer: function(row) {
                                ...
                                if((<?=$this->session->userdata['ast_user']['user_id']?>==dataRecord.legal_user || <?=$this->session->userdata['ast_user']['user_system_admin_sts']?>=='2') && (dataRecord.cma_sts == 63 || dataRecord.cma_sts == 61 || dataRecord.cma_sts == 69 || dataRecord.cma_sts == 67 || dataRecord.cma_sts == 62) ){
                        ...
                        <? if(ADDITIONALINPUT==1){?>
                          { text: 'ADI', datafield: 'additionalinput', editable: false,align:'center', sortable: false, menu: false, width: 35,
                            cellsrenderer: function(row) {
                                ...
                                if((<?=$this->session->userdata['ast_user']['user_id']?>==dataRecord.legal_user || <?=$this->session->userdata['ast_user']['user_system_admin_sts']?>=='2') && (dataRecord.cma_sts == 63 || dataRecord.cma_sts == 69 || dataRecord.cma_sts == 62)){
                        ...
                        <? if(SENDFORAUTHORIZATION==1){?>
                          { text: 'SFA', datafield: 'sendforauthorization', editable: false,align:'center', sortable: false, menu: false, width: 35,
                            cellsrenderer: function(row) {
                                ...
                                if((<?=$this->session->userdata['ast_user']['user_id']?>==dataRecord.legal_user || <?=$this->session->userdata['ast_user']['user_system_admin_sts']?>=='2') && dataRecord.total_auth<1 && dataRecord.ln_exp_sts==1 && (dataRecord.cma_sts == 63 || dataRecord.cma_sts ==62 || dataRecord.cma_sts==69)){
                        ...
                        <? if(EDIT==1){?>
                          { text: 'EC', datafield: 'edit', editable: false,align:'center', sortable: false, menu: false, width: 35,
                            cellsrenderer: function(row) {
                                editrow = row;
                                var dataRecord = jQuery("#jqxgrid").jqxGrid('getrowdata', editrow);
                                if((<?=$this->session->userdata['ast_user']['user_id']?>==dataRecord.legal_user || <?=$this->session->userdata['ast_user']['user_system_admin_sts']?>=='2' || <?=$this->session->userdata['ast_user']['user_work_group_id']?>=='10' || <?=$this->session->userdata['ast_user']['user_work_group_id']?>=='11') && dataRecord.cma_sts!=64 && dataRecord.cma_sts!=65 && dataRecord.cma_sts!=75){
                        ...
                        { text: 'Branch Name', datafield: 'branch_name',editable: false, width: '10%', align:'left',cellsalign:'left'},
    ...
    function details(id,operation,indx,cif,proposed_type,sec_sts)
    {
        ...
        jQuery.ajax({
            ...
            data : {[csrfName]: csrfHash,id: id,
            district:'',zone:'',operation:operation
            },
            datatype: "json",
            success: function(response){
            //alert(response);
                ...
                jQuery('.txt_csrfname').val(json.csrf_token);
                    if(json.str)
                    {
                        ...
                        if(operation=='sendnotificationlawyer' && json.court_fee>0)
                        {
                            ...
                            {
                                var html = '';
                                html+='<input name="new_court_fee_sts" id="new_court_fee_sts" value="1" type="hidden"><input style="float:left;width:100px" name="court_fee_amount" type="text" id="court_fee_amount" value="'+json.court_fee.toFixed(2)+'" onkeypress="return numbersonly(event)" class="text-input-big" />&nbsp;<span style="float:left"><strong>procurement:</strong></span>';
    ...
    function get_user_data_region_wise(){
      ...
      var item2 = jQuery("#branch_sol").jqxComboBox('getSelectedItem');
      var zone = '';
      var branch = '';
      ...
      async:false,
      type: "post",
     data: { [csrfName]: csrfHash,zone : zone,branch : branch},
      datatype: "json",
      success: function(response){
          var json = jQuery.parseJSON(response);
          jQuery('.txt_csrfname').val(json.csrf_token);
                ...
                <div id="reassign_row" style="text-align:center;margin-bottom: 20px;font-family:calibri;font-size:15px;">
                    <div style="margin-bottom: 20px;margin-top:20px;font-family:calibri;font-size:15px;">
                        <table style="margin-left: 310px;margin-top: 0px;">
                            <tr>
                                <td>Zone:</td>
                                <td><div id="zone" name="zone" style="padding-left: 3px" onchange="get_user_data_region_wise()"></div></td>
                            </tr>
                            <tr>
                                <td>Branch:</td>
                                <td><div id="branch_sol" name="branch_sol" style="padding-left: 3px" onchange="get_user_data_region_wise()"></div></td>
                            </tr>
                            <tr>
