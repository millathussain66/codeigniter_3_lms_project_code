...
<script type="text/javascript">
	...
	function take_action()
	{
		if (row_validation())
		{
			...
			jQuery("#type").val('bulk_create');
			jQuery("#message").html('Create');
			jQuery("#button_tag").html('Create');
			jQuery("#email_notification").val('create');
			jQuery("#success_message").val('Created');
	...
	{
		var row_count=0;
		if(parseInt(jQuery("#total_number_of_error").val())>0)
		{
			alert('Sorry you have some error For Submit');
			return false;
		}
...
<body style="font-family:calibri">    
	...
	<div align="left">	
		...
		<form class="form" id="form" name="form"  method="post" action="<?=base_url()?>authorization_request/file_validation" enctype="multipart/form-data">
					...
					<table id="deal_body" style="display:block;width:100%">
						<tr>
							...
							<td  style="text-align:center;width:50%">
							  <strong>Download Templete File : </strong> <a id="inXLadfc" style="margin-left:10px;text-align:center;cursor:pointer;" href="<?=base_url()?>authorization_request/templete" target="_blank" ><img align="center" src="<?=base_url()?>images/download_002.png"></a>
							</td>
						</tr>
					</table>
			  </div>
