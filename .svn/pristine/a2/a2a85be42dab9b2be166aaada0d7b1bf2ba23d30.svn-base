...
class Warrant_arrest extends CI_Controller {
	...
	function mk_xl_pending_report(){
		$where2 = "s.sts=1 AND ((s.case_sts_prev_dt=39 AND s.req_type=1) OR (s.case_name = 4 AND s.req_type =2 AND s.case_sts_prev_dt=56))";

		if($this->input->post('s_case_number')!='') 
		{
			$where2.=" AND s.case_number = '".trim($this->input->post('s_case_number'))."'";
		}
		if($this->input->post('proposed_type')!='') 
		{$where2.=" AND s.proposed_type = '".trim($this->input->post('proposed_type'))."'";}
		if($this->input->post('account')!='') 
		{
			if($this->input->post('proposed_type')=='Card'){
				...
				{$where2.=" AND s.loan_ac = '".trim($this->input->post('account'))."'";}
			}
		}
		$this->db->select('s.case_number,s.case_claim_amount,s.case_claim_amount,s.proposed_type,s.loan_ac,s.ac_name,
    	z.name as zone_name,bs.name as branch_name,rq.name as case_type, d.name as district_name,
    	cs.name as case_sts
    	', FALSE)
			->from("suit_filling_info s")
			->join('ref_zone z', 'z.id=s.zone', 'left')
			->join('ref_district d', 'd.id=s.district', 'left')
			->join('ref_req_type rq', 'rq.id=s.req_type', 'left')
			->join('ref_branch_sol bs', 'bs.id=s.branch_sol', 'left')
    ...
    $colnum = count($headings2);
        ...
        $objPHPExcel->getActiveSheet()->getStyle('A'.$rowNumber.':'.xl_col($colnum-1).$rowNumber)->getFill()->applyFromArray(array('type' => PHPExcel_Style_Fill::FILL_SOLID, 'startcolor' => array('rgb' => 'F28A8C')));
        $rowNumber++;
        $counter=0;
    ...
    $objPHPExcel->setActiveSheetIndex(0);
        $objPHPExcel->getActiveSheet()->setTitle('WA Pending Report'); 
        //include_once(dirname(dirname(__FILE__)).DIRECTORY_SEPARATOR.'Classes'.DIRECTORY_SEPARATOR.'PHPExcel'.DIRECTORY_SEPARATOR.'IOFactory.php');
        require_once './application/Classes/PHPExcel/IOFactory.php';
        $objWriter = PHPExcel_IOFactory::createWriter($objPHPExcel, 'Excel5');//Excel2007
        ...
        header("Content-Type:   application/vnd.ms-excel; charset=utf-8");
        header("Content-type:   application/x-msexcel; charset=utf-8");
        header('Content-Disposition: attachment;filename="warrent_arrest_pending.xls"');
        header("Expires: 0");
        header("Cache-Control: must-revalidate, post-check=0, pre-check=0");
        header("Cache-Control: private",false);
        $objWriter->save('php://output'); 
  ...
  function mk_xl_executed_report(){
		$where2 = "e.sts=1 AND e.v_sts=38";

		if($this->input->post('case_number')!='') 
		{
			$where2.=" AND s.case_number = '".trim($this->input->post('case_number'))."'";
		}
		if($this->input->post('s_arrested_by')!='' && $this->input->post('s_arrested_by')!=0) 
		{$where2.=" AND e.arrested_by = '".trim($this->input->post('s_arrested_by'))."'";}
		...
		$this->db->select('if(e.status=1,"Pending","Setteled") as fsts, 
			s.case_number,s.case_claim_amount,s.proposed_type,s.loan_ac,s.ac_name,
    	z.name as zone_name,d.name as district_name,bs.name as branch_name,
    	e.debtor_name,rq.name as case_type,cs.name as case_name,fr.name as final_remarks,
    	cr.name as court_name,DATE_FORMAT(s.prev_date,"%d-%m-%Y") as date_judgment,e.fine_amount,
    	e.impresonment_month,"Executed" as warrent_status,e.remarks
    	', FALSE)
			->from("file_executed_data e")
			->join("suit_filling_info s",'s.id=e.file_id', 'left')
			->join('ref_zone z', 'z.id=s.zone', 'left')
			->join('ref_final_remarks fr', 'fr.id=s.final_remarks', 'left')
			->join('ref_court cr', 'cr.id=s.prest_court_name', 'left')
			->join('ref_district d', 'd.id=s.district', 'left')
			->join('ref_case_name cs', 'cs.id=s.case_name', 'left')
			->join('ref_req_type rq', 'rq.id=s.req_type', 'left')
			->join('ref_branch_sol bs', 'bs.code=s.branch_sol', 'left')
			->where($where2);
		$result=$this->db->get()->result();
    require_once('./application/Classes/PHPExcel.php'); 
        ...
        $objPHPExcel->getActiveSheet()->getColumnDimension('R')->setWidth(15);
		    $headings2 = array(
		    	...
		    	'District',
		    	'Branch',
		    	'Account No (CD/SND/MSD)',
		    	'Account Name',
		    	'Accused/Judgement Debtor Name',
		    	'Case/Suit Type',
		    	'Case/Suit Name',
		    	'Suit No',
		    	'Suit Status',
    ...
    $colnum = count($headings2);
      ...
      $dataar = array(
                  ...
                  $row->case_claim_amount,
                  $row->fine_amount,
                  $row->impresonment_month,
                  $row->warrent_status,
                  $row->remarks,
              );
      $objPHPExcel->getActiveSheet()->fromArray($dataar,NULL,'A'.$rowNumber);
      if(in_array('Account No (CD/SND/MSD)',$headings2)){
                $key = array_search ('Account No (CD/SND/MSD)', $headings2);
                $objPHPExcel->getActiveSheet()->setCellValueExplicit((xl_col($key).$rowNumber), $acnum, PHPExcel_Cell_DataType::TYPE_STRING);
            }
      $rowNumber++;
    }
    ...
    $objPHPExcel->setActiveSheetIndex(0);
        $objPHPExcel->getActiveSheet()->setTitle('WA Executed Report'); 
        //include_once(dirname(dirname(__FILE__)).DIRECTORY_SEPARATOR.'Classes'.DIRECTORY_SEPARATOR.'PHPExcel'.DIRECTORY_SEPARATOR.'IOFactory.php');
        require_once './application/Classes/PHPExcel/IOFactory.php';
        $objWriter = PHPExcel_IOFactory::createWriter($objPHPExcel, 'Excel5');//Excel2007
        ...
        header("Content-Type:   application/vnd.ms-excel; charset=utf-8");
        header("Content-type:   application/x-msexcel; charset=utf-8");
        header('Content-Disposition: attachment;filename="warrent_arrest_executed.xls"');
        header("Expires: 0");
        header("Cache-Control: must-revalidate, post-check=0, pre-check=0");
        header("Cache-Control: private",false);
        $objWriter->save('php://output'); 
