...
<script type="text/javascript">
	...
	jQuery(document).ready(function () {
		...
		<?php if ($this->session->userdata['ast_user']['user_work_group_id']=='1'): ?>
			change_dropdown('legal_region',<?=$this->session->userdata['ast_user']['region']?>);
			jQuery("#legal_region").jqxComboBox('val', '<?=$this->session->userdata["ast_user"]["region"]?>');
        ...
        success: function(response){
        	var json = jQuery.parseJSON(response);
					...
					if (operation=='legal_district_case_deal_officer') 
					{
						var case_deal_officer = [];
						jQuery.each(json['row_info'],function(key,obj){
							case_deal_officer.push({ value: obj.id, label: obj.name+'('+obj.user_id+')' });
							//alert(obj.name);
						});
